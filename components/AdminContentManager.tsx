...
export function AdminContentManager({ kind, initial }: { kind: AdminContentKind; initial: AdminContentRecord[] }) {
  ...
  const upload = async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]; if (!file || !editing) return; setSaving(true); setMessage("กำลังอัปโหลดรูป…");
    try { const data = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result).split(",")[1] || ""); reader.onerror = () => reject(new Error("อ่านไฟล์ไม่สำเร็จ")); reader.readAsDataURL(file); }); const response = await fetch("/.netlify/functions/admin-upload", { method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ filename: file.name, mimeType: file.type, data, folder: `${kind}-${editing.slug || "new"}` }) }); const body = await response.json().catch(() => ({})); if (!response.ok || !body.url) throw new Error(body.error || "อัปโหลดรูปไม่สำเร็จ"); setEditing({ ...editing, ...(kind === "articles" ? { coverImage: body.url } : { image: body.url }) }); setMessage("อัปโหลดรูปแล้ว กรุณากดบันทึก"); }
    catch (reason) { setMessage(reason instanceof Error ? reason.message : "อัปโหลดรูปไม่สำเร็จ"); } finally { setSaving(false); event.target.value = ""; }
  };
  const blank: AdminContentRecord = { slug: "", title: "", description: "", enabled: true };
  return <section className="admin-panel"><div className="admin-panel-heading"><div><p>{kind.toUpperCase()}</p><h2>{labels[kind].title}</h2></div><button className="is-primary" type="button" onClick={() => { setMessage(""); setEditing(blank); }}>＋ {labels[kind].add}</button></div>{message && <p className="admin-form-message" role="status">{message}</p>}<div className="admin-card-list">{records.map((record) => <article key={record.slug}><span>{record.number || "•"}</span><div><h3>{record.title}</h3><p>{record.description || record.excerpt}</p></div><button type="button" onClick={() => setEditing({ ...record })}>แก้ไข</button></article>)}</div>
